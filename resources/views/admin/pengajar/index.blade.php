...
@push('styles')
    <style>
        ...
        .stat-icon-wrapper.pengajar { background: #10B981; color: #FFFFFF; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3); }
    ...
    <div class="content-canvas">
        ...
        <div class="page-header">
            <div>
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-active">Data Guru</span>
                </div>
                <h2 class="header-title">Data Guru</h2>
                <p class="header-subtitle">Kelola informasi guru, penugasan, dan status keaktifan pengajar TPA.</p>
            </div>
        ...
        <div class="table-section">
                ...
                <tbody>
                    @forelse($pengajars as $p)
                        <tr>
                            ...
                            <td>
                                <span class="role-badge role-guru">GURU PENGAJAR</span>
                            </td>
                            <td>
                                @if($p->id_user)
                                    <span
                            ...
                            <td style="text-align: right;">
                                ...
                                @php
                                    /** @var \App\Models\Pengajar $p */
                                    $namaStr = addslashes($p->nama);
                                    $alamatStr = addslashes($p->alamat);
                                    $emailStr = $p->user ? addslashes($p->user->email) : '';
                                    
                                    $onAkun = 'onclick="openAkunModal(' . $p->id_pengajar . ', \'' . $namaStr . '\', \'guru\')"';
                                    $onReset = 'onclick="openAkunModal(' . $p->id_pengajar . ', \'' . $namaStr . '\', \'guru\', \'' . $emailStr . '\')"';
                                    $onEdit = 'onclick="openEditModal(' . $p->id_pengajar . ', \'' . $namaStr . '\', \'' . $p->jenis_kelamin . '\', \'' . $p->no_hp . '\', \'' . $alamatStr . '\')"';
                                    $onDelete = 'onclick="openDeleteModal(' . $p->id_pengajar . ')"';
                                @endphp
                                @if(!$p->id_user)
                                    <button type="button" class="btn-primary"
                                        style="padding: 6px 12px; font-size: 12px; gap: 4px;" {!! $onAkun !!}>
                                        <span class="material-symbols-outlined" style="font-size: 14px;">person_add</span> Buat
                                        Akun
                                    </button>
                                @else
                                    <button type="button" class="btn-secondary"
                                        style="padding: 6px 12px; font-size: 12px; gap: 4px;" {!! $onReset !!}>
                                        <span class="material-symbols-outlined" style="font-size: 14px;">lock_reset</span> Reset
                                        Password
                                    </button>
                                @endif
                                <button type="button" class="btn-icon" {!! $onEdit !!} title="Edit Data">
                                    <span class="material-symbols-outlined" style="font-size: 18px;">edit</span>
                                </button>
                                <button type="button" class="btn-icon btn-icon-danger" {!! $onDelete !!} title="Hapus Data">
            ...
            <div class="pagination-container">
                <div class="pagination-info">Menampilkan
                    {{ $pengajars->firstItem() ?? 0 }}-{{ $pengajars->lastItem() ?? 0 }} dari {{ $pengajars->total() }} data
                </div>
                <div class="pagination-controls">
                    ...
                    @else
                        <a href="{{ $pengajars->appends(request()->query())->previousPageUrl() }}"
                            class="page-btn outline"><span class="material-symbols-outlined"
                                style="font-size:18px">chevron_left</span></a>
                    @endif
                    
                    <!-- Pagination Elements -->
                    @php
                        $start = max($pengajars->currentPage() - 2, 1);
                        $end = min($pengajars->currentPage() + 2, $pengajars->lastPage());
                    @endphp
                    
                    @if($start > 1)
                        <a href="{{ $pengajars->url(1) }}" class="page-btn">1</a>
                        @if($start > 2)
                    ...
                    @for($page = $start; $page <= $end; $page++)
                        @if($page == $pengajars->currentPage())
                            <button class="page-btn active">{{ $page }}</button>
                        @else
                            <a href="{{ $pengajars->url($page) }}" class="page-btn">{{ $page }}</a>
                        @endif
                    @endfor

                    @if($end < $pengajars->lastPage())
                        @if($end < $pengajars->lastPage() - 1)
                            <button class="page-btn" disabled>...</button>
                        @endif
                        <a href="{{ $pengajars->url($pengajars->lastPage()) }}" class="page-btn">{{ $pengajars->lastPage() }}</a>
                    @endif

                    <!-- Next Page Link -->
                    @if($pengajars->hasMorePages())
                        <a href="{{ $pengajars->appends(request()->query())->nextPageUrl() }}" class="page-btn outline"><span
        ...
        <div id="deleteModal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(4px);">
            <div style="background: white; border-radius: 24px; padding: 32px; width: 90%; max-width: 400px; position: relative; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04);">
                ...
                <h3 style="font-family: 'Epilogue', sans-serif; font-weight: 700; font-size: 20px; color: #003227; margin: 0 0 16px 0;">Hapus Data Guru</h3>
                
                <p style="font-family: 'Plus Jakarta Sans', sans-serif; font-size: 14px; color: #64748B; margin: 0 0 32px 0; line-height: 1.6;">
                    Apakah Anda yakin ingin menghapus data guru ini? Tindakan ini bersifat permanen dan tidak dapat dibatalkan.
                </p>
...
@push('scripts')
    <script>
        ...
        function openEditModal(id, nama, jk, hp, alamat) {
        ...
        function openAkunModal(id, nama, role, email = '') {
            ...
            form.action = `{{ url('/admin/pengajar') }}/${id}/akun`;
